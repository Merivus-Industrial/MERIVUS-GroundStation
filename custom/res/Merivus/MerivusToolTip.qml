...
Item {
    ...
    property real maximumWidth: Math.min(360, ScreenTools.defaultFontPixelWidth * 42)

    // 内部边距，用于提示文字与边框之间的空白。
    readonly property real _padding: 8

    // 组件宽度：先计算文本宽度加左右边距，再与最小宽度比较，最后不超过最大宽度。
    width: Math.min(Math.max(minimumWidth, tipMetrics.advanceWidth + (_padding * 2)), maximumWidth)
    ...
    QGCLabel {
        ...
        y: root._padding
        text: root.text
        width: root.width - (root._padding * 2)
        color: qgcPal.text
        font.pixelSize: 11
