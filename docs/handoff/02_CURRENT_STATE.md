...
| --- | --- |
| 更新时间 | 2026-08-19 |
| 集成分支 | `main`；短期功能分支经 PR 合并后删除 |
| 本轮审计基线 | `9bb0807`（整理开始时的 `origin/main`） |
| 工作区 | 以 `git status --short` 和当前 PR 为准；长期文档不固化瞬时脏文件清单 |
| 当前阶段 | `0.1.0-dev.1` 开发测试初版已收口，进入安全收敛与可重复验证阶段 |
| 当前优先任务 | 非 AI 高风险执行入口统一安全收敛、PX4 起飞健康检查和 Mock/SITL 回归基线 |
...
| QGC Release 构建 | qmake/VS 脚本存在；历史记录称增量构建通过 | `a40c9e4`；[版本说明](../releases/0.1.0-dev.1.md) | 2026-08-19 环境检查通过；本机 MSVC 19.44 在重建 PCH 时异常退出，未形成新的可验证 Windows 产物；等待 GitHub CI 与匹配工具链复验 |

## 正在进行

- PX4 引导起飞已收敛为每机独立的异步预检、起飞 ACK、解锁前复检与拒绝原因刷新状态机；等待 MockLink/SITL 时序回归。
- 工程继续补齐非 AI 高风险入口的统一确认边界与可重复验证；历史 roadmap 中的“进行中”仍需以代码和当前 PR 为准。
