...
foreach ($path in @($qmake, $jom, $vcvars, $project)) {
...
if ($Reconfigure -or -not (Test-Path -LiteralPath $makefile)) {
...
$binary = Join-Path $buildDir "$Configuration\MERIVUS.exe"
if (-not (Test-Path -LiteralPath $binary)) {
    $binary = Join-Path $buildDir "MERIVUS.exe"
}

if (Test-Path -LiteralPath $binary) {
    Write-Host "Build succeeded: $binary"
} else {
    Write-Warning "Build command succeeded but MERIVUS.exe was not found at the expected paths."
}
